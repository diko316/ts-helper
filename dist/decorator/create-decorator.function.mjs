import { DECORATOR_TYPE_STATIC_PARAMETER, DECORATOR_TYPE_PARAMETER, DECORATOR_TYPE_STATIC_ACCESSOR, DECORATOR_TYPE_ACCESSOR, DECORATOR_TYPE_STATIC_METHOD, DECORATOR_TYPE_METHOD, DECORATOR_TYPE_STATIC_PROPERTY, DECORATOR_TYPE_PROPERTY, DECORATOR_TYPE_CLASS } from './create-decorator.constant.mjs';
import { getDecoratorCallInfo } from './get-decorator-call-info.function.mjs';

function createDecorator({ settings, classDecorator, propertyDecorator, methodDecorator, accessorDecorator, parameterDecorator, staticPropertyDecorator, staticMethodDecorator, staticAccessorDecorator, staticParameterDecorator, }) {
    function decorator(...callParams) {
        const info = getDecoratorCallInfo(callParams);
        switch (info === null || info === void 0 ? void 0 : info.type) {
            case DECORATOR_TYPE_CLASS:
                if (classDecorator) {
                    return classDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            case DECORATOR_TYPE_PROPERTY:
                if (propertyDecorator) {
                    return propertyDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            case DECORATOR_TYPE_STATIC_PROPERTY:
                if (staticPropertyDecorator) {
                    return staticPropertyDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            case DECORATOR_TYPE_METHOD:
                if (methodDecorator) {
                    return methodDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            case DECORATOR_TYPE_STATIC_METHOD:
                if (staticMethodDecorator) {
                    return staticMethodDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            case DECORATOR_TYPE_ACCESSOR:
                if (accessorDecorator) {
                    return accessorDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            case DECORATOR_TYPE_STATIC_ACCESSOR:
                if (staticAccessorDecorator) {
                    return staticAccessorDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            case DECORATOR_TYPE_PARAMETER:
                if (parameterDecorator) {
                    return parameterDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            case DECORATOR_TYPE_STATIC_PARAMETER:
                if (staticParameterDecorator) {
                    return staticParameterDecorator({
                        ...info,
                        settings: settings,
                    });
                }
                return;
            default:
                return;
        }
    }
    return decorator;
}

export { createDecorator };
//# sourceMappingURL=create-decorator.function.mjs.map
